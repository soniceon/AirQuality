...
export default function Home() {
  ...
  function getCityKeyByLabel(label: string): string {
    const resources = i18n.options.resources || {};
    const lng = i18n.language;
    let cities: any = (resources as any)?.[lng]?.home?.cities || {};
    if (!cities || typeof cities !== 'object') {
      const tCities = t('home.cities', { returnObjects: true });
      if (typeof tCities === 'object') cities = tCities;
    }
    if (typeof cities === 'object') {
      for (const key in cities) {
        if (cities[key] === label) return key;
      }
    }
    let enCities: any = (resources as any)?.['en']?.home?.cities || {};
    if (!enCities || typeof enCities !== 'object') {
      const tEnCities = t('home.cities', { lng: 'en', returnObjects: true });
      if (typeof tEnCities === 'object') enCities = tEnCities;
    }
    if (typeof enCities === 'object') {
      for (const key in enCities) {
        if (enCities[key] === label) return key;
      }
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (loc && loc.city) {
          const key = getCityKeyByLabel(loc.city);
          setCity(key);
          fetchAirQuality(key, true);
        }
        if (loc && loc.country) {
      ...
      <div className="bg-white rounded-xl shadow-lg p-8">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-4">
            ...
            <p className="text-gray-600">{t('home.intro')}</p>
        </div>
        <div className="mb-8">
          <div className="flex gap-4 max-w-md mx-auto">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                value={city ? t(`home.cities.${city}`, city) : ''}
                onChange={(e) => {
                  const input = e.target.value.trim();
                  if (!input) {
                    setCity('');
                  } else {
                    const key = getCityKeyByLabel(input);
                    setCity(key);
                  }
                }}
